...
class TestReportScreen extends StatefulWidget {
  const TestReportScreen({super.key, required this.sessionId, required this.sectionId});
...
class _TestReportScreenState extends State<TestReportScreen> {
  @override
  void initState() {
    context.read<ResultCubit>().calculateResult(widget.sessionId, widget.sectionId);
    super.initState();
  }
      ...
      body: BlocBuilder<ResultCubit, ResultState>(
        builder: (context, state) {
          ...
          if (state is ResultLoaded) {
            final questions = (context.read<TestQuestionsCubit>().state as TestQuestionsLoaded).questions;
            int totalXp = 0;
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                        children: [
                          ...
                          for (int i = 0; i < state.testResult.questionResults.length; i++)
                            ReportTile(
                              title: "Question ${i + 1}",
                              description:
                                  questions[i].question,
                              xpGained: state.testResult.questionResults[i].obtainedXp,
                              ...
                              onTap: (){
                                GoRouter.of(context).pushNamed(
                                  ...
                                  extra: {
                                    ...
                                    'questionResult': state.testResult.questionResults[i]
                                  }
                                );
                              },
                            )
